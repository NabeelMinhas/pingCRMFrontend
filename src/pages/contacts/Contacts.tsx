...
export default function Contacts() {
  ...
  const { data, isLoading, refetch } = useQuery<PaginatedResponse>({
    queryKey: ['contacts', search, currentPage, filterStatus],
    queryFn: async () => {
      const params: any = { 
        search,
      ...
      // Add company_id filter if provided (for filtering contacts by company)
      if (formData.companyId) {
        params.company_id = formData.companyId;
      }
  ...
  const restoreContact = useMutation({
    mutationFn: async (id: number) => {
      return axios.post(`/api/contacts/${id}/restore`);
    },
    onSuccess: () => {
      refetch();
    },
  ...
  const handleSoftDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this contact?')) {
      try {
        await softDeleteContact.mutateAsync(id);
      } catch (error) {
            ...
            <div className="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium leading-6 text-gray-900">Create Contact</h3>
                  <button
              ...
              <tbody className="divide-y divide-gray-200">
                {isLoading ? (
                  ...
                  data.items.map((contact: Contact) => (
                    <tr key={contact.id} className={contact.deletedAt ? "bg-gray-50" : ""}>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                        {contact.deletedAt ? (
                          ...
                          <>
                            <Link to={`/contacts/${contact.id}`} className="text-indigo-600 hover:text-indigo-900 mr-4">
                              <span className="sr-only">Edit {contact.firstName} {contact.lastName}</span>
                              <span>Edit</span>
                            </Link>
                            <button
